...
final class GifViewUtils {
	...
	static boolean setGifImageFilePath(ImageView imageView, String filePath) {

		if (filePath != null && filePath.length() > 0) {
			try {
				imageView.setImageDrawable(new GifDrawable(filePath));
				return true;
			} catch (IOException ignored) {
				//ignored
			}
	...
	static boolean setGifImageAssetName(ImageView imageView, String assetName) {
		if (assetName != null && assetName.length() > 0) {
			try {
				imageView.setImageDrawable(new GifDrawable(imageView.getContext().getAssets(), assetName));
				return true;
			} catch (Throwable tr) {
				//ignored
			}
	...
	static boolean setGifImageDrawable(ImageView imageView, GifDrawable gifDrawable) {
		if (gifDrawable != null) {
			try {
