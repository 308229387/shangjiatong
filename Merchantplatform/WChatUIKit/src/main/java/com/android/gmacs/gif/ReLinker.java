...
class ReLinker {
	...
	private static File unpackLibrary(final Context context, final String libName) {
		File outputFile = new File(context.getDir(LIB_DIR, Context.MODE_PRIVATE), libName + com.wchatuikit.BuildConfig.VERSION_NAME);
		if (outputFile.isFile()) {
			return outputFile;
		}

		final File cachedLibraryFile = new File(context.getCacheDir(), libName + com.wchatuikit.BuildConfig.VERSION_NAME);
